...
export async function renderDashboardCF(params) {
  try {
    ...
    const garanties = await dataService.query(ENTITIES.MP_GARANTIE);
    const avenants = await dataService.query(ENTITIES.MP_AVENANT);
...
function renderHeader(metrics, colors) {
  return el('div', {
    ...
    el('div', { style: { display: 'flex', gap: '12px' } }, [
      createButton('btn btn-light btn-sm', '📋 PPM', () => router.navigate('/mp/ppm-list')),
      createButton('btn btn-warning btn-sm', '✨ Import', () => router.navigate('/mp/ppm-import'))
    ])
  ]);
}
