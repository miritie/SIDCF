...
export async function renderRecours(params) {
  ...
  // Load recours
  const allRecours = await dataService.query(ENTITIES.MP_RECOURS, { operationId: idOperation });
...
async function handleAddRecours(idOperation) {
  ...
  const result = await dataService.add(ENTITIES.MP_RECOURS, recoursData);
  ...
  // Reload page
  router.navigate('/mp/recours', { idOperation });
}
