...
// .wrangler/tmp/bundle-YHyoLa/checked-fetch.js
...
// .wrangler/tmp/bundle-YHyoLa/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-YHyoLa/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-YHyoLa/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function updateEntity(entityType, id, patch, env) {
  ...
  const setClause = Object.keys(snakePatch).filter((k2) => k2 !== "id" && k2 !== "created_at").map((col) => {
    ...
    if (val === null || val === void 0)
      return `${col} = NULL`;
    if (typeof val === "object")
      return `${col} = '${JSON.stringify(val)}'::jsonb`;
    if (typeof val === "string")
